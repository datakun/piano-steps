...
export default function ChordCard({ root, quality, voicingMode }: ChordCardProps) {
  const voicing = useMemo(
    () => voicingMode === 'drop2'
      ? buildDrop2Voicing(root, quality)
      : buildBasicVoicing(root, quality),
    [root, quality, voicingMode]
  );
  ...
  const highlights = useMemo<NoteHighlight[]>(() => {
    ...
    return all.map((p, i) => {
      const label = i < voicing.leftHand.length
        ? (i === 0 ? '1' : 'D2')
        : degreeLabels[i - voicing.leftHand.length] ?? '';
      return {
        note: p.name,
        octave: p.octave,
